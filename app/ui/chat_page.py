...
def _render_help_panels():
    _, col_faq, col_questions = st.columns([6, 1, 2])
    with col_faq:
        if st.button("FAQ"):
...
def _handle_user_input(graph, user_input: str):
        ...
        with st.spinner("Thinking..."):
            ...
            try:
                ...
                if "ResourceExhausted" in error_text or "429" in error_text or "quota" in error_text.lower():
                    result = GraphState(
                        response=(
                            "Provider quota exceeded (429). Please retry later, adjust billing/quota, "
    ...
    logger.info("request_id=%s | Assistant response preview=%s", request_id, _log_preview(assistant_response))

    assistant_box.write(assistant_response)
    st.session_state.messages.append({"role": "assistant", "content": assistant_response})


def _render_upload_widget(project_root: Path):
    if not st.session_state.awaiting_upload:
        return

    uploaded = st.file_uploader("Upload RFP PDF", type=["pdf"], key="rfp_pdf_uploader")
    if not uploaded:
        return
    ...
    st.session_state.messages.append(
        {"role": "assistant", "content": f"Uploaded `{uploaded.name}` successfully. Ask your RFP question now."}
    )
    st.rerun()


def render_chat_page(graph, project_root: Path):
    ...
    if user_input:
        _handle_user_input(graph, user_input)

    _render_upload_widget(project_root)
